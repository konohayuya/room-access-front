    ...
    <form v-on:submit.prevent="formSubmit">
      <p>
        <label for="student-id">
          <font-awesome-icon :icon="['fas', 'id-card']" fixed-width class="fa"/>学生番号
        </label><br>
      ...
      <p>
        <label for="temperature">
          <font-awesome-icon :icon="['fas', 'thermometer-three-quarters']"
                             fixed-width class="fa" />体温
        </label><br>
        <select id="temperature" v-model="temperature">
          <option v-for="(n, i) in 31" :key="35 + i/10" :value="35 + i/10">
            {{(35 + i / 10).toFixed(1)}}
          </option>
        </select>
      </p>
      <button type="submit" id="enter-button">
        <font-awesome-icon :icon="['fas', 'sign-in-alt']" fixed-width class="fa"/> 入室
      </button>
    </form>
  </div>
</template>
